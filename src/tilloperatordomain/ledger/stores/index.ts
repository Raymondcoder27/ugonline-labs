...
export const useBilling = defineStore("billing", () => {
  ...
  async function fetchFloatLedgers() {
    const { data } = await api.get("/till-operator5-float-ledgers");
  ...
  async function fetchFloatRequests() {
    const { data } = await api.get("/till-operator5-float-requests");
  ...
  async function requestFloat(payload: RequestFloat) {
    const { data } = await api.post("/till-operator5-float-requests", {
      amount: payload.amount,
      // tillId: payload.tillId,
      till: payload.till,
      status: "pending",
  ...
  //   const { data } = await api.post("/till-operator5-float-ledgers", {
  //     amount: payload.amount,
  //     description: payload.description,
  //     status: "pending",
  //   });
