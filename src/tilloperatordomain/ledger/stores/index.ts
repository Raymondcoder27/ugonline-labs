...
export const useBilling = defineStore("billing", () => {
  ...
  async function fetchFloatLedgers() {
    const { data } = await api.get("/till-operator3-float-ledgers");
  ...
  async function requestFloat(payload: RequestFloat) {
    const { data } = await api.post("/till-operator3-float-requests", {
      amount: payload.amount,
      // tillId: payload.tillId,
      till: payload.till,
      status: "pending",
  ...
  //   const { data } = await api.post("/till-operator3-float-ledgers", {
  //     amount: payload.amount,
  //     description: payload.description,
  //     status: "pending",
  //   });
