...
export const useBalance = defineStore("balance", () => {
  ...
  async function updateTotalBalance(payload: {
    ...
    if (payload.status === "pending") {
      ...
      // const {data} = await api.post("/till-operator3-float-request-amount", payload.amount);
      const { data } = await api.post("/till-operator3-float-request-amount", {
    ...
    if (payload.status === "approved") {
      ...
      //post that float request to the api
      const { data } = await api.post("/till-operator3-float-request-amounts", {
        tillId: payload.tillId,
        amount: payload.amount,
        description: payload.description
      });
