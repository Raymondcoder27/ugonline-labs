...
export const useBilling = defineStore("billing", () => {
  ...
  //   const { data } = await api.post("/branch7-manager-float-requests", {
  ...
  async function requestFloatToAdmin(payload: RequestFloatToAdmin) {
    try {
      ...
      const ledgerResponse = await api.post("/branch7-manager-float-ledgers", ledgerEntry);
  ...
  //     const { data } = await api.post(`/branch7-manager-float-ledgers`, {
  //       // id: floatLedgers.value.length + 1,
  //       date: new Date().toISOString(),
  //       // till: payload.till,
  //       // branch: payload.branch,
  ...
  //   const { data } = await api.put("/till-operator17-float-requests/" + requestId, {
  //     status: "approved",
  //     approvedBy: "Manager One",
